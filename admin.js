...
/*ADMIN*/
function hello(){
    var user = JSON.parse(localStorage.getItem('userlogin'));// trả giá trị userlogin trong localstorage vào user
    document.getElementById('hello').innerHTML= user.fullname +'<button onclick="logout()">Đăng xuất</button>';
    //trả về họ tên kèm đăng xuất
}
function logout(){
    localStorage.removeItem('userlogin');
    localStorage.removeItem('cart');
    location.href='watchvn.html';
}
...
function showchangeproductbox(productid){
    ...
    for(var i=0;i<productArray.length;i++){
        if(productArray[i].productId == productid){
            ...
            document.getElementById('imgafter').src="./img/temp.jpeg";
...
function changeproduct(productid){// doi san pham trong localstorage
    document.getElementById('modal1').style.display = 'none';
    var productArray = JSON.parse(localStorage.getItem('product'));
    var vitri;
    for(var i=0;i<productArray.length;i++){
        if(productArray[i].productId == productid){// chỗ click vào có id đó (productid)
            ...
            vitri = (Math.floor(i/10))*10;//vi tri san pham can sua (trang)
        }
    }
    localStorage.setItem('product', JSON.stringify(productArray));// truyền vào localstorage
    showProductList(vitri);
}
function changeimg(input){// phần sửa sp
...
function changeimgadd(input){ //phần thêm sản phẩm
    var reader = new FileReader();
    reader.onload = function (e) {
...
function addProduct(){
    ...
    var producttemp = {productId: productid, brand: brand.value, img:'img/hinhtam.png', name: productname.value, price: price.value};
    productArray.unshift(producttemp);// thêm vào đầu mảng producttemp
    localStorage.setItem('product',JSON.stringify(productArray));
    showProductList(0);
...
function customAlert(message,type) {
    ...
    if (type=='warning') {
        document.getElementById("customalert").style.backgroundColor = 'red';
    }
