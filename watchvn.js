...
function showProduct(){
	...
	if(temp[1]=='' || temp[1]==undefined || temp[1].search('all')==0){// neu sau"?" ko xac dinh hoac = 0
		...
		var temp2 = temp.split("&");// all&0 --> all
 		var vitri = temp2[1];// 0
		var sotrang=0,dem=0;
		...
		for(var i = 1;i<=sotrang;i++){
			vitri=(i-1)*12;
			var a ='<a href="watchvn.html?all&'+vitri+'">'+i+'</a>';
...
function updateCart(quantity,id){ // cập nhật 
	var cartArray = JSON.parse(localStorage.getItem('cart'));
	for (var i = 0; i < cartArray.length; i++) {
		if(cartArray[i].productId==id){
...
function search(){
	...
	var s='';
	if(document.getElementById('searchextend').className== ''){ // tìm kiếm cơ bản
...
function showextend2(){ // tìm kiếm trong giỏ hàng
	if(document.getElementById('searchextend').className== ''){
		document.getElementById('searchextend').classList.add('active')
		document.querySelector('#searchsection .searchbox button img').src = 'img/up.svg';
	}
...
function createUser(e){
	e.preventDefault();// huỷ sự kiện nếu có thể huỷ // đăng ký xong sẽ thoát ra form đăng ký
...
function login(e){// khi đăng nhập xong nó thotas khỏi form login
	e.preventDefault();
